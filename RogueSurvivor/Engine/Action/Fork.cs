    ...
    class Fork : ActorAction,BackwardPlan
    {
      ...
      public Fork(Actor actor, Dictionary<Location, ActorAction>? legal_path) : base(actor)
      {
        ...
        foreach(var x in legal_path) {
          if (x.Value is Engine.Actions.ActorDest) m_Options.Add(new Engine.Actions.ActionMoveDelta(m_Actor, x.Key));
          else if (x.Value is Engine.Actions.Resolvable act) m_Options.Add(act.ConcreteAction);
      ...
      public List<ActorAction>? prequel() {
        ...
        while(0 <= --ub) {
          if (m_Options[ub] is BackwardPlan act) {
            ...
            if (null != setup) foreach (var x in setup) {
              var test = new Engine.Actions.ActionChain(m_Actor, new List<ActorAction> { x, m_Options[ub] });
        ...
        while(lb < preview_ret.Count-1) {
          ...
          if (null != fork_options) preview_ret[lb] = new Engine.Actions.ActionChain(m_Actor, new List<ActorAction> { preview_ret[lb].ConcreteAction, new Fork(m_Actor, fork_options) });
          lb++;
        }
