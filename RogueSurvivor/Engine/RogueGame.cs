...
#nullable enable
    ...
    {
      if (null == s_witnesses && null == o_witnesses) return;
      var so_witnesses = s_witnesses?.Intersect(o_witnesses);
      s_witnesses = s_witnesses?.SetDifference(so_witnesses);
      o_witnesses = o_witnesses?.SetDifference(so_witnesses);
    ...
    private static UI.Message MakeMessage<T>(ActorController viewpoint, Actor actor, string doWhat, T target, string phraseEnd = ".") where T : ILocation, INoun
    {
      var msg = new string[] { viewpoint.VisibleIdentity(actor), doWhat, viewpoint.VisibleIdentity(target)+phraseEnd };
        ...
        if (actorList1 != null) {
          foreach (Actor actor in actorList1) {
            ...
            if (!actor.Model.Abilities.IsUndead && Session.Get.HasImmediateZombification && rules.RollChance(s_Options.StarvedZombificationChance)) {
              ...
              witnesses?.ImportantMessage(MakePanopticMessage(actor, string.Format("{0} into a Zombie!", "turn".Conjugate(actor))), DELAY_LONG);
            }
          }
        }
#endregion
...
#nullable restore
    ...
    {
      var a_witness = _ForceVisibleToPlayer(a);
      a.SpendActionPoints();
      int num = a.DamageVsCorpses;
      if (null != a_witness) {
        a_witness.AddMessage(MakePanopticMessage(a, string.Format("{0} {1} corpse.", VERB_FEAST_ON.Conjugate(a), c.DeadGuy.Name)));
      ...
      if (c.TakeDamage(num)) {
        a.Location.Map.Destroy(c);
        a_witness?.AddMessage(new(string.Format("{0} corpse is no more.", c.DeadGuy.Name), a.Location.Map.LocalTime.TurnCounter, Color.Purple));
      }
...
#nullable restore
    ...
    private bool HandlePlayerDirectiveFollower(Actor player, OrderableAI ordai)    // XXX could be void return but other HandlePlayer... return bool
    {
      ...
      do {
        ...
        AddMessage(new(string.Format("1. {0} grenades.", directives.CanThrowGrenades ? "Throw" : "Don't throw"), Session.Get.WorldTime.TurnCounter, Color.LightGreen));
        AddMessage(new(string.Format("2. {0}.", directives.CanSleep ? "Sleep" : "Don't sleep"), Session.Get.WorldTime.TurnCounter, Color.LightGreen));
        AddMessage(new(string.Format("3. {0}.", directives.CanTrade ? "Trade" : "Don't trade"), Session.Get.WorldTime.TurnCounter, Color.LightGreen));
        RedrawPlayScreen(new UI.Message(string.Format("4. {0}.", directives.Courage.to_s()), Session.Get.WorldTime.TurnCounter, Color.LightGreen));
        KeyEventArgs keyEventArgs = m_UI.UI_WaitKey();
        int choiceNumber = KeyToChoiceNumber(keyEventArgs.KeyCode);
...
#nullable restore
    ...
    public void DoMoveActor(Actor actor, in Location newLocation)
    {
      ...
      if (draggedCorpse != null) {
        ...
        a_witness?.AddMessage(msg);
        d_only_witness?.AddMessage(msg);
      }
      ...
      if (null != mapObjectAt && mapObjectAt.IsJumpable) {
        ...
        a_witness?.AddMessage(MakePanopticMessage(actor, VERB_JUMP_ON.Conjugate(actor), mapObjectAt));
        if (actor.Model.Abilities.CanJumpStumble && Rules.Get.RollChance(JUMP_STUMBLE_CHANCE)) {
          actionCost += JUMP_STUMBLE_ACTION_COST;
          a_witness?.AddMessage(MakePanopticMessage(actor, string.Format("{0}!", VERB_STUMBLE.Conjugate(actor))));
        }
      }
    ...
    private bool DoTriggerTrap(ItemTrap trap, Actor victim)
    {
      ...
      if (dmg > 0) {
        ...
        if (null != witnesses) {
          ...
          witnesses.ImportantMessage(MakePanopticMessage(victim, string.Format("is hurt by {0} for {1} damage!", trap.AName, dmg)), victim.IsPlayer ? DELAY_NORMAL : DELAY_SHORT);
          ClearOverlays();
          RedrawPlayScreen();
        }
      }
...
#nullable restore
      ...
      if (hitRoll > defRoll) {
        ...
        if (attacker.Model.Abilities.CanDisarm && rules.RollChance(attack.DisarmChance)) {
          ...
          if (null != disarmIt) {
            ...
            if (null != d_witness) {
              d_pc?.Messages.Clear();
              ad_witness?.AddMessage(MakeMessage(ad_witness[0], attacker, VERB_DISARM.Conjugate(attacker), defender));
              d_only_witness?.AddMessage(MakeMessage(d_only_witness[0], attacker, VERB_DISARM.Conjugate(attacker), defender));
              d_witness.AddMessage(new(string.Format("{0} is sent flying!", disarmIt.TheName), attacker.Location.Map.LocalTime.TurnCounter));
              if (null != d_pc) {
        ...
        if (dmg > 0) {
          ...
          if (fatal) {
            if (isVisible) {
              ...
              ad_witness?.ImportantMessage(MakeMessage(ad_witness[0], attacker, (defender.Model.Abilities.IsUndead ? VERB_DESTROY : (Rules.IsMurder(attacker, defender) ? VERB_MURDER : VERB_KILL)).Conjugate(attacker), defender, " !"), DELAY_LONG);
              a_only_witness?.ImportantMessage(MakeMessage(a_only_witness[0], attacker, (defender.Model.Abilities.IsUndead ? VERB_DESTROY : (Rules.IsMurder(attacker, defender) ? VERB_MURDER : VERB_KILL)).Conjugate(attacker), defender, " !"), DELAY_LONG);
              d_only_witness?.ImportantMessage(MakeMessage(d_only_witness[0], attacker, (defender.Model.Abilities.IsUndead ? VERB_DESTROY : (Rules.IsMurder(attacker, defender) ? VERB_MURDER : VERB_KILL)).Conjugate(attacker), defender, " !"), DELAY_LONG);
            }
            ...
            if (Session.Get.HasImmediateZombification) {
              if (to_immediately_zombify) {
                ...
                if (isDefVisible) {
                  ad_witness?.ImportantMessage(MakeMessage(ad_witness[0], attacker, "turn".Conjugate(attacker), defender, " into a Zombie!"), DELAY_LONG);
                  d_only_witness?.ImportantMessage(MakeMessage(d_only_witness[0], attacker, "turn".Conjugate(attacker), defender, " into a Zombie!"), DELAY_LONG);
                }
              }
            ...
            ad_witness?.ImportantMessage(MakeMessage(ad_witness[0], attacker, attack.Verb.Conjugate(attacker), defender, string.Format(" for {0} damage.", dmg)), isPlayer ? DELAY_NORMAL : DELAY_SHORT);
            d_only_witness?.ImportantMessage(MakeMessage(d_only_witness[0], attacker, attack.Verb.Conjugate(attacker), defender, string.Format(" for {0} damage.", dmg)), isPlayer ? DELAY_NORMAL : DELAY_SHORT);
          }
        } else if (isVisible) {
          if (display_defender) AddOverlay(new OverlayImage(MapToScreen(defender.Location), GameImages.ICON_MELEE_MISS));
          ad_witness?.ImportantMessage(MakeMessage(ad_witness[0], attacker, attack.Verb.Conjugate(attacker), defender, " for no effect."), isPlayer ? DELAY_NORMAL : DELAY_SHORT);
          a_only_witness?.ImportantMessage(MakeMessage(a_only_witness[0], attacker, attack.Verb.Conjugate(attacker), defender, " for no effect."), isPlayer ? DELAY_NORMAL : DELAY_SHORT);
          d_only_witness?.ImportantMessage(MakeMessage(d_only_witness[0], attacker, attack.Verb.Conjugate(attacker), defender, " for no effect."), isPlayer ? DELAY_NORMAL : DELAY_SHORT);
        }
      }
      else if (isVisible) {
        if (display_defender) AddOverlay(new OverlayImage(MapToScreen(defender.Location), GameImages.ICON_MELEE_MISS));
        ad_witness?.ImportantMessage(MakeMessage(ad_witness[0], attacker, VERB_MISS.Conjugate(attacker), defender), isPlayer ? DELAY_NORMAL : DELAY_SHORT);
        a_only_witness?.ImportantMessage(MakeMessage(a_only_witness[0], attacker, VERB_MISS.Conjugate(attacker), defender), isPlayer ? DELAY_NORMAL : DELAY_SHORT);
    ...
    public void UI_ThrowGrenadeUnprimed(Actor actor, Point targetPos, ItemGrenade itemGrenade, List<PlayerController>? a_witness, List<PlayerController>? d_witness)
    {
      ...
      a_witness?.ImportantMessage(MakePanopticMessage(actor, phrase), DELAY_LONG);
      d_only_witness?.ImportantMessage(MakeMessage(d_only_witness[0], actor, phrase), DELAY_LONG);
      ClearOverlays();
      RedrawPlayScreen();
    }
...
#nullable restore
    ...
    public void DoGiveItemTo(Actor actor, Actor target, Item gift)
    {
      ...
      (new Data.Model.InvOrigin(actor)).Transfer(gift, new Data.Model.InvOrigin(target));
...
#nullable enable
    ...
    public void DoUseEntertainmentItem(Actor actor, ItemEntertainment ent)
    {
      ...
      witnesses?.AddMessage(MakePanopticMessage(actor, VERB_ENJOY.Conjugate(actor), ent));
      int boreChance = ent.Model.BoreChance;
      if (boreChance == 100) {
        actor.Inventory!.Consume(ent);
        witnesses?.AddMessage(MakePanopticMessage(actor, VERB_DISCARD.Conjugate(actor), ent));
      } else if (Rules.Get.RollChance(boreChance)) {
        ent.AddBoringFor(actor);
        witnesses?.AddMessage(MakePanopticMessage(actor, string.Format("{0} now bored of {1}.", VERB_BE.Conjugate(actor), ent.TheName)));
      }
...
#nullable restore
      ...
      if (mapObj is DoorWindow doorWindow && doorWindow.IsBarricaded) {
        ...
        if (isVisible) {
          ...
          if (flag) {
            ...
            ad_witness?.ImportantMessage(MakeMessage(ad_witness[0], actor, VERB_BREAK.Conjugate(actor), mapObj), DELAY_LONG);
            a_only_witness?.ImportantMessage(MakeMessage(a_only_witness[0], actor, VERB_BREAK.Conjugate(actor), mapObj), DELAY_LONG);
            d_only_witness?.ImportantMessage(MakeMessage(d_only_witness[0], actor, VERB_BREAK.Conjugate(actor), mapObj), DELAY_LONG);
          } else {
            if (player1) AddOverlay(new OverlayImage(MapToScreen(actor.Location), GameImages.ICON_MELEE_ATTACK));
            if (player2) AddOverlay(new OverlayImage(MapToScreen(mapObj.Location), GameImages.ICON_MELEE_DAMAGE));
            ad_witness?.ImportantMessage(MakeMessage(ad_witness[0], actor, VERB_BASH.Conjugate(actor), mapObj), actor.IsPlayer ? DELAY_NORMAL : DELAY_SHORT);
            a_only_witness?.ImportantMessage(MakeMessage(a_only_witness[0], actor, VERB_BASH.Conjugate(actor), mapObj), actor.IsPlayer ? DELAY_NORMAL : DELAY_SHORT);
            d_only_witness?.ImportantMessage(MakeMessage(d_only_witness[0], actor, VERB_BASH.Conjugate(actor), mapObj), actor.IsPlayer ? DELAY_NORMAL : DELAY_SHORT);
          }
          ClearOverlays();
        }
    ...
    public void DoWakeUp(Actor actor)
    {
      ...
      var witnesses = _ForceVisibleToPlayer(actor);
      witnesses?.AddMessage(MakePanopticMessage(actor, string.Format("{0}.", VERB_WAKE_UP.Conjugate(actor))));
    ...
    private void DoTag(Actor actor, ItemSprayPaint spray, Point pos)
    {
      ...
      var witnesses = _ForceVisibleToPlayer(actor);
      witnesses?.AddMessage(MakePanopticMessage(actor, string.Format("{0} a tag.", VERB_SPRAY.Conjugate(actor))));
    }
...
#nullable enable
    ...
    {
      var witnesses = _ForceVisibleToPlayer(actor);
      witnesses?.AddMessage(MakeMessage(actor, string.Format("{0} {1}.", VERB_SPRAY.Conjugate(actor), (sprayOn == actor ? actor.HimselfOrHerself : sprayOn.Name))));
    }
    ...
    public void KillActor(Actor killer, Actor deadGuy, string reason)
    {
      ...
      if (null != killer) {
        ...
        if (Session.Get.HasEvolution && killer.Model.Abilities.IsUndead) {
          ...
          if (actorModel != null) {
            ...
            if (null != witnesses) {
              ...
              witnesses.ImportantMessage(MakePanopticMessage(killer, string.Format("{0} a {1} horror!", VERB_TRANSFORM_INTO.Conjugate(killer), actorModel.Name)), DELAY_LONG);
              ClearOverlays();
            }
          }
        }
        ...
        if (isMurder.cache) {
          ...
          var witnesses = _ForceVisibleToPlayer(killer);
          witnesses?.AddMessage(MakePanopticMessage(killer, string.Format("murdered {0}!!", deadGuy.Name)));
...
#nullable enable
    private void DoLooseRandomSkill(Actor actor)
    {
      ...
      if (null != lost) {
        var witnesses = _ForceVisibleToPlayer(actor);
        witnesses?.AddMessage(MakePanopticMessage(actor, string.Format("regressed in {0}!", Skills.Name(lost.Value))));
      }
    }
...
#nullable enable
    private void CloseAllGates(Map map,string gate_name)
    {
      ...
      map.DoForAllMapObjects(obj => {
          ...
          witnesses?.AddMessage(MakePanopticMessage(actorAt, string.Format("{0} {1} crushed between the closing " + gate_name + "!", VERB_BE.Conjugate(actorAt))));
      });
    }
#nullable restore
...
#nullable enable
  static internal class RogueGame_ext {
    ...
    static public bool AddMessage(this List<PlayerController> witnesses, UI.Message msg)
    {
        bool rendered = false;
    ...
    static public bool ImportantMessage(this List<PlayerController> witnesses, UI.Message msg, int delay=0)
    {
      bool rendered = false;
