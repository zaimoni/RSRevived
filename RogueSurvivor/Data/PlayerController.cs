...
#region UI messages
    ...
    public void AddMessagePressEnter()
    {
      ...
      var game = RogueGame.Game;
      game.PanViewportToRedraw(m_Actor.Location);
    ...
    public override bool ReportBlocked(in Model.InvOrigin src, Actor who) {
    ...
    public override bool ReportGone(in Model.InvOrigin src, Actor who) {
      var code = CommunicationMethodCode(who);
      if (0 >= code) return false;
    ...
    public override bool ReportNotThere(in Model.InvOrigin src, Gameplay.Item_IDs what, Actor who) {
      var code = CommunicationMethodCode(who);
      if (0 >= code) return false;
    ...
    public override bool ReportTaken(in Model.InvOrigin src, Item it, Actor who)  {
      var code = CommunicationMethodCode(who);
      if (0 >= code) return false;
